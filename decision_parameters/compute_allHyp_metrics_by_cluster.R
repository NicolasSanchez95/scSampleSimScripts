#!/usr/bin/env Rscript
# Per-(simulation, cluster) all-hypothesis FDP / power.
#
# Like compute_allHyp_metrics.R, but aggregates via
# get_conf_metrics_tested_all_hypotheses_by_cluster.

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 7) {
  stop("Usage: Rscript compute_allHyp_metrics_by_cluster.R <repo_root> <cohort_name> <overlap_def> <cut_off_true> <cut_off_false> <sig_threshold_fixed> <p_val_col_name>")
}
...
saved_with_params <- paste0(
  "conf_metrics_tested_by_cluster_", overlap_def, "_", cut_off_true, "_", cut_off_false, "_",
...
for (phen_type_removal in phen_type_removal_set) {
  ...
  conf_metrics_tested <- scSampleSim::get_conf_metrics_tested_all_hypotheses_by_cluster(
